...
This repository contains a script to benchmark anomaly detection models on the MVTec AD dataset. It develops an extension of the normal benchmark by addind an all classes concatenated training.
...
Run all categories/models (each class trained separately):

```bash
python testbench.py
```

Run all categories/models (all classes concatenated):

```bash
python testbench.py --train-together
```
